...
class Conveyor(AgentThread):
    ...
    def loop_body(self):
        ...
        if self.read_from_shared('process', None) in ['main', 'ceiling_robot_duration']:
            if self.locals['state'] == "STOPPED" and not self.read_from_sensor('Proximity.object_detected'):
                self.write_to_actuator('ConveyorBelt.control_power', 100.0)
            ...
            if self.locals['state'] == "STARTED" and self.read_from_sensor('Proximity.object_detected'):
                ...
                self.write_to_shared('conv_part_pose', None, self.read_from_sensor('Proximity.part_loc')[0])
