...
OR : '||' | 'or';
NOT : '!';
fragment LID : [a-z][a-zA-Z0-9_]*; //difference betwee lid and cid???

fragment CID : [A-Z][a-zA-Z0-9_]*;
UPPER : CID;
...
program :  NEWLINE? defs  module*   (allreadvars | allwritevars | localvars)*   init?  event+ EOF
        | NEWLINE? defs (allreadvars | allwritevars )* init? (AGENT LPAR UPPER RPAR NEWLINE* module* localvars? event+ )+ EOF
        | NEWLINE? defs (allreadvars | allwritevars )*  module* localvars? init? event+ EOF;
defs : funcdef*  adtdef* ;
